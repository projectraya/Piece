class ThreeSphere {
    ...
    async initialize(canvasId, analyser, dataArray) {
        ...
        // Вземи реалните размери
        const w = canvas.clientWidth || window.innerWidth;
        const h = canvas.clientHeight || window.innerHeight;
        canvas.width = w;
        canvas.height = h;
        console.log('[ThreeSphere] Canvas size set to:', w, h);
    ...
    animate() {
        ...
        if (!this.sphere || !this.analyser || !this.dataArray || !this.renderer) return;
    ...
    detectBeats() {
        ...
        const now = Date.now();
        if (now - this.lastBeatTime > 50) {
            if (bass > 120) { this.createImpact('bass', -0.35); this.lastBeatTime = now; }
            else if (lowMid > 110) { this.createImpact('lowmid', -0.25); this.lastBeatTime = now; }
            else if (mid > 100) { this.createImpact('mid', 0.2); this.lastBeatTime = now; }
            else if (highMid > 90) { this.createImpact('highmid', 0.18); this.lastBeatTime = now; }
            else if (treble > 80) { this.createImpact('treble', 0.15); this.lastBeatTime = now; }
        }
    }
    ...
    createImpact(type, strength) {
        ...
        this.impacts.push({
            ...
            strength, radius: 0.4, time: 0, duration: 0.4, type
        });
    }
    ...
    updateSphereWithImpacts() {
        ...
        for (let i = this.impacts.length - 1; i >= 0; i--) {
            this.impacts[i].time += 1 / 60;
            if (this.impacts[i].time >= this.impacts[i].duration) this.impacts.splice(i, 1);
        }
        ...
        for (let i = 0; i < positions.length; i += 3) {
            ...
            let totalDeformation = Math.sin(time * 0.4 + nx * 2) * 0.02 + Math.cos(time * 0.3 + ny * 3) * 0.015;

            for (let impact of this.impacts) {
                const dx = nx - impact.x, dy = ny - impact.y, dz = nz - impact.z;
                const dist = Math.sqrt(dx * dx + dy * dy + dz * dz);
                if (dist < impact.radius) {
                    const curve = Math.sin((impact.time / impact.duration) * Math.PI);
                    totalDeformation += impact.strength * curve * (1 - dist / impact.radius);
                }
            }
    ...
    updateColors() {
        ...
        for (let i = 0; i < colors.length; i += 3) {
            ...
            const hue = ((Math.atan2(z, x) + Math.PI) / (Math.PI * 2) * 360 + time * 20) % 360;
